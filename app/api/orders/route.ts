...
const seen=new Set();for(const i of items){if(!i||typeof i.variantId!=='string'||!Number.isInteger(i.quantity)||i.quantity<1||i.quantity>20||(i.type!==undefined&&(typeof i.type!=='string'||i.type.length>100)))throw new HttpError(400,'পণ্যের পরিমাণ সঠিক নয়।');const dk=i.variantId+'|'+(i.type||'');if(seen.has(dk))throw new HttpError(400,'পণ্যের পরিমাণ সঠিক নয়।');seen.add(dk)}
const requestHash=await hash(JSON.stringify({customerName:customerName.trim(),phone:normalizedPhone,address:address.trim(),area,notes:notes.trim(),items:[...items].sort((a,b)=>(a.variantId+(a.type||'')).localeCompare(b.variantId+(b.type||'')))}));
...
const lines=items.map(i=>{const v=rows.results.find(v=>v.id===i.variantId);if(!v)throw new HttpError(400,'পণ্যটি এখন পাওয়া যাচ্ছে না।');return {...v,name:i.type||v.name,quantity:i.quantity}});
